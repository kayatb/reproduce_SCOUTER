...
def parse_bounding_box(row):
    ...
    del s[0::5]  # Delete the first and every fifth element from the list (these contain the label for the bounding box).
    s = [int(i) for i in s]
    it = iter(s)

    return list(zip(it, it, it, it))  # Group each four elements together (i.e. one bounding box)
            ...
            for img in os.listdir(path):
                ...
                if not os.path.isdir(img_path):
                    ...
                    for box in orig_bboxes[img]:
                        ...
                        new_box = (int(np.round(x_scale * x1)), 
...
if __name__ == '__main__':
    ...
    parser.add_argument("--csv_loc", type=str, required=True, help="Location of the CSV with the bounding box coordinates")
